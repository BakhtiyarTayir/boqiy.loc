...
// add_action('admin_init', 'test_make_product_variation');

function test_make_product_variation() {
    // Замените 31345 на ID вашего продукта для тестирования
    $product_id = 31261;
    ...
    if (!$product) {
        ...
        add_action('admin_notices', function() use ($product_id) {
            echo '<div class="notice notice-error is-dismissible">
                    <p>Не удалось получить товар с ID: ' . $product_id . '</п>
                  </div>';
        });
        return;
    }

    // Вызов функции для добавления атрибута и создания вариаций
    make_product_variation($product);

    // Сообщение в админке для подтверждения выполнения
    add_action('admin_notices', function() use ($product) {
        echo '<div class="notice notice-success is-dismissible">
                <p>Продукт успешно преобразован в вариативный с атрибутами и вариациями.'. $product->get_regular_price() . '</p>
              </div>';
    });
}
...
function make_product_variation($product) {
    ...
    if ($found_attribute) {
        ...
        foreach ($languages as $locale) {
            if ($locale != 'uz') { // Предполагая, что основной язык - английский
