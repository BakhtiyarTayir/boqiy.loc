...
function move_product_to_free_category($order_id) {
    ...
    foreach ($order->get_items() as $item) {
        ...
        if (has_term(array('homiylik', 'sponsorskij'), 'product_cat', $product_id)) {
            // Убираем товар из категории "Спонсорский"
            wp_remove_object_terms($product_id, 'homiylik', 'product_cat');
            wp_remove_object_terms($product_id, 'sponsorskij', 'product_cat');

            // Определяем язык товара и добавляем его в соответствующую категорию
            if (function_exists('pll_get_post_language')) {
                $product_lang = pll_get_post_language($product_id);
                if ($product_lang == 'uz') {
                    wp_set_object_terms($product_id, 'tekin-mahsulotlar', 'product_cat', true);
                } elseif ($product_lang == 'ru') {
                    wp_set_object_terms($product_id, 'tovary-dlya-ballov', 'product_cat', true);
                }
            }
            ...
            // Получаем переводы товара и делаем то же самое
            if (function_exists('pll_get_post_translations')) {
                $translations = pll_get_post_translations($product_id);
                foreach ($translations as $lang => $translated_product_id) {
                    if ($translated_product_id != $product_id) { // Пропускаем основной язык
                        $translated_product = wc_get_product($translated_product_id);
                        wp_remove_object_terms($translated_product_id, 'homiylik', 'product_cat');
                        wp_remove_object_terms($translated_product_id, 'sponsorskij', 'product_cat');
                        
                        if ($lang == 'uz') {
                            wp_set_object_terms($translated_product_id, 'tekin-mahsulotlar', 'product_cat', true);
                        } elseif ($lang == 'ru') {
                            wp_set_object_terms($translated_product_id, 'tovary-dlya-ballov', 'product_cat', true);
                        }
                        make_product_variation($translated_product);
                    }
                }
            }
        }
...
function make_product_variation($product) {
    ...
    if ($found_attribute) {
        ...
        $terms = array('Стандартная Оплата', 'Оплата бонусом');
