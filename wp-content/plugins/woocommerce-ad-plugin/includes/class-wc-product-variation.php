...
class WC_Product_Variation_Copy {
    const FREE_CATEGORY_SLUG = 'tekin-mahsulotlar'; // Замените на слаг вашей категории

    public function __construct() {


        add_action('woocommerce_thankyou', [$this, 'move_product_to_free_category'], 10, 1);
    ...
    public function move_product_to_free_category($order_id) {
        if (!$order_id) return;
        ...
        foreach ($order->get_items() as $item) {
            ...
            if (has_term('homiylik', 'product_cat', $product_id)) {
                // Извлекаем данные формы из сессии 
                $ad_data = WC()->session->get('ad_data_' . $product_id);
                
                WC()->session->__unset('ad_data_' . $product_id); 
                // Делаем товар вариативным
                $this->make_product_variation($product, $ad_data);
            }
        } 
    } 


    public function make_product_variation($product, $ad_data) {
        ...
        if ($found_attribute) {
            ...
            $terms = ['Оплата бонусом', 'Стандартная Оплата'];
            ...
            // Сохраняем данные формы в мета-данные нового продукта
            if ($ad_data) {
                foreach ($ad_data as $key => $value) {
                    update_post_meta($new_product->get_id(), $key, $value); 
                    // var_dump($key, $value);
                }
            ...
            wp_set_object_terms($new_product->get_id(), self::FREE_CATEGORY_SLUG, 'product_cat', false);

            // Создаем вариации
            $this->create_product_variations($new_product->get_id(), $attribute_taxonomy, $terms, $product->get_regular_price());
            ...
            foreach ($languages as $locale) {
                if ($locale != 'uz') { // Предполагая, что основной язык - узбекский
                    $translated_product_data = [
                        'name' => $product->get_name() , // Пример переведенного имени продукта
                    ...
                    $this->create_product_translation($new_product->get_id(), $translated_product_data, $locale, $new_category, $attributes);
                }
            }
    ...
    public function create_product_variations($product_id, $attribute_taxonomy, $terms, $base_price) {
        $new_price = $base_price * 1.15;
        foreach ($terms as $term) {
            $term_slug = get_term_by('name', $term, $attribute_taxonomy)->slug;
            $variation = new WC_Product_Variation();
            $variation->set_parent_id($product_id);
            $variation->set_attributes([$attribute_taxonomy => $term_slug]);
    ...
    public function create_product_translation($product_id, $translated_product_data, $locale, $new_category, $attributes) {
        ...
        $original_variations = wc_get_products([
            'parent' => $product_id,
            'type' => 'variation',
        ]);
