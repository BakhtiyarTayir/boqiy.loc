...
class BPDev_Account_Auto_Activator {
	...
	public static function login_redirect( $user_login, $password ) {
		...
		if (  self::use_email_login( $user_login ) ) {
			$ud = get_user_by( 'login', $user_login );
			if ( $ud ) {
				$creds = array( 'user_login' => $ud->user_email, 'user_password' => $password );
			}
		} else {
	...
	private static function get_redirect_url( $user ) {
		...
		$redirect_url = empty( $settings['redirect_url'] ) ? bp_core_get_user_domain( $user->ID ) : self::get_parsed_url( $user, $settings['redirect_url'] );
