...
class Youzify_Premium_Ajax {
	...
	function get_share_activity_form() {
	    ...
	    // Send Result.
		wp_send_json_success( array( 'activity_id' => $activity_id, 'form' => $form, 'posts_emojis' => youzify_option( 'youzify_enable_posts_emoji', 'on' ), 'preview' => $preview, 'show_all' => '<div class="youzify-show-all-less"><div class="youzify-show-all"><i class="fas fa-arrow-down"></i>' . __( 'Show All', 'youzify' ) . '</div><div class="youzify-collapse"><i class="fas fa-arrow-up"></i>' . __( 'Collapse', 'youzify' ) . '</div></div>' ) );
	...
	function form_content() {
		...
		add_action( 'youzify_wall_before_submit_form_action', array( $this, 'post_in_button' )  );
	...
	function handle_sticky_posts() {
		...
		$sticky_posts = youzify_option( 'youzify_' . $component . '_sticky_posts' );
