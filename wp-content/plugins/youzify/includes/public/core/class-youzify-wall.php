...
class Youzify_Wall {
    ...
    function add_comment_attachments( $content ) {
		...
		if ( ! empty( $attachments ) ) {
			...
			switch ( $type ) {
				...
				echo apply_filters( 'youzify_get_wall_post_video', '<video width="100%" controls preload="metadata" type="video/quicktime"><source src="' . wp_get_attachment_url( $attachment_id ) . '" type="video/mp4">' . __( 'Your browser does not support the video tag.', 'youzify' ) . '</video>', $attachments, $attachment_id, $data );
					break;
	...
	function get_wall_post_file( $attachments ) {
		...
		<div class="youzify-wall-file-post">
			...
			<a rel="nofollow" href="<?php echo wp_get_attachment_url( $media_id ); ?>" class="youzify-wall-file-download"><i class="fas fa-download"></i><?php _e( 'Download', 'youzify' ); ?></a>
		</div>
	...
	function get_wall_post_video( $attachments ) {
		...
		if ( ! empty( $attachments ) ) {
			...
			foreach ( $attachments as $media_id => $data ) {
				...
				echo apply_filters( 'youzify_get_wall_post_video', '<video width="100%" controls preload="metadata" type="video/quicktime" poster="' . $poster . '"><source src="' . wp_get_attachment_url( $media_id ) . '" type="video/mp4">' . __( 'Your browser does not support the video tag.', 'youzify' ) . '</video>', $media_id, $data );
			}
