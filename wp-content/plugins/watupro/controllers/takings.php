...
function watupro_takings($in_shortcode = false, $atts = null) {	
	...
	$filters_url="dn=".esc_attr(@$_GET['dn'])."&dnf=".esc_attr(@$_GET['dnf'])."&email=".esc_attr(@$_GET['email'])."&emailf=".
		...
		"&phonef=".esc_attr(@$_GET['phonef'])."&field_phone=".esc_attr(@$_GET['field_phone'])."&ip=".esc_attr(@$_GET['ip'])."&ipf=".esc_attr(@$_GET['ipf'])."&date=".esc_attr(@$_GET['date']).
		"&date2=".esc_attr(@$_GET['date2'])."&datef=".esc_attr(@$_GET['datef'])."&points=".esc_attr(@$_GET['points'])."&pointsf=".esc_attr(@$_GET['pointsf']).
		"&grade=".esc_attr(@$_GET['grade'])."&role=".esc_attr(@$_GET['role'])."&ugroup=".esc_attr(@$_GET['ugroup'])."&quiz_cat_id=".esc_attr($_GET['quiz_cat_id']).
		"&percent_correct=".esc_attr(@$_GET['percent_correct'])."&percentf=".esc_attr(@$_GET['percentf'])."&source_url=".esc_attr(@$_GET['source_url']).
		"&in_progress=".$in_progress."&per_page=".$per_page."&loggedin=".esc_attr(@$_GET['loggedin']);	
	if(!empty($namaste_courses) and !empty($_GET['namaste_course_id'])) {
		$filters_url .= "&namaste_course_id=".intval($_GET['namaste_course_id']);
	}		
		
	$display_filters=(!count($filters) and !count($joins) and empty($role_join_sql) and empty($_GET['ugroup'])) ? false : true;
