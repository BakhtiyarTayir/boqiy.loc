...
if ( ! class_exists( 'YITH_WCAF_Commission_Handler' ) ) {
	...
	class YITH_WCAF_Commission_Handler {
		...
		protected $_commission_order_status_map = array(
			'not-confirmed'   => array( 'on-hold', 'processing' ),
			'pending'         => array( 'completed' ),
		...
		public function __construct() {
			...
			$this->_status_labels_map                  = apply_filters( 'yith_wcaf_commission_status_labels_map', array(
				...
				'will-delivery'         => __( 'Will deliver', 'yith-woocommerce-affiliates' ),
				'than-take'         => __( 'Then take', 'yith-woocommerce-affiliates' ),
				'archive'         => __( 'Archive', 'yith-woocommerce-affiliates' ),
				...
				'not-confirmed'   => __( 'New order', 'yith-woocommerce-affiliates' ),
		...
		public function map_commission_status( $order_status ) {
			...
			return apply_filters( 'yith_wcaf_default_commission_status', 'not-confirmed', $order_status );
		}
			...
			if ( ! empty( $_REQUEST['commissions'] ) ) {
				...
				switch ( $current_action ) {
					...
					case 'switch-to-not-confirmed':
						...
						break;
					case 'switch-to-archive':
							foreach ( $_REQUEST['commissions'] as $commission_id ) {
								$res      = YITH_WCAF_Commission_Handler()->change_commission_status( $commission_id, 'archive' );
								$redirect = esc_url_raw( add_query_arg( 'commission_status_change', $res, $redirect ) );
							}
						break;
						case 'switch-to-than-take':
							foreach ( $_REQUEST['commissions'] as $commission_id ) {
								$res      = YITH_WCAF_Commission_Handler()->change_commission_status( $commission_id, 'than-take' );
								$redirect = esc_url_raw( add_query_arg( 'commission_status_change', $res, $redirect ) );
							}
						break;

						case 'switch-to-will-delivery':
							foreach ( $_REQUEST['commissions'] as $commission_id ) {
								$res      = YITH_WCAF_Commission_Handler()->change_commission_status( $commission_id, 'will-delivery' );
								$redirect = esc_url_raw( add_query_arg( 'commission_status_change', $res, $redirect ) );
							}
						break;
