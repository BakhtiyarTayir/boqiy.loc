...
if ( ! class_exists( 'YITH_WCAF_Shortcode' ) ) {
	...
	class YITH_WCAF_Shortcode {
		...
		public static function init() {
			...
			add_shortcode( 'yith_wcaf_show_statistics', array( 'YITH_WCAF_Shortcode', 'show_stats' ) );
		...
		public static function affiliate_dashboard( $atts = array() ) {
			...
			elseif ( isset( $wp->query_vars['stats'] ) ) {
				$return = self::affiliate_dashboard_stats( $atts );
			}  // otherwise, show summary

			elseif ( isset( $wp->query_vars['products'] ) ) {
				$return = self::affiliate_dashboard_products( $atts );
			}  // otherwise, show summary
			else {
		 ...
		 * @since 1.0.0
		 */
		public static function affiliate_dashboard_products( $atts = array() ) {
			...
			/**
			 ...
			 $template_name = 'dashboard-products.php';
		 ...
		 * @since 1.0.0
		 */
		public static function affiliate_dashboard_stats( $atts = array() ) {
		   ...
		   /**
			...
			$template_name = 'dashboard-stat.php';
		...
		public static function affiliate_dashboard_settings( $atts = array() ) {
			...
			if ( ! empty( $_REQUEST['payment_email'] ) && apply_filters( 'yith_wcaf_is_payment_email',  $_REQUEST['payment_email'], $_REQUEST['payment_email'] ) ) {
				$payment_email = apply_filters( 'yith_wcaf_sanitized_payment_email', $_REQUEST['payment_email'], $_REQUEST['payment_email'] );
		...
		public static function affiliate_dashboard_summary( $atts = array() ) {
			...
			$payment_email = isset( $_POST['payment_email'] ) ? apply_filters( 'yith_wcaf_sanitized_payment_email',  wp_unslash( $_POST['payment_email'] ), $_POST['payment_email'] ) : $affiliate['payment_email'];
